...
export default function App() {
  ...
  useEffect(() => {
    const id = generateId();
    const task = {
      id, label: 'T1', index: 1, role: 'master',
      iframeRef: createRef(),
  ...
  function handleAdd() {
    ...
    const task = {
      id, label: `T${index}`, index, role: 'sub',
      iframeRef: createRef(),
  ...
  async function handleBroadcast(command, targetIds, senderTaskId) {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      try {
        ...
        receipt = { ok: res.ok, sent: res.sent ?? targets.length, failed: res.failed ?? [] };
      } catch {
  ...
  function handleTopBarBroadcast(command) {
    ...
    const targetIds = activeTask.role === 'master'
      ? subTasks.map(t => t.id)
  ...
  useEffect(() => {
    function handler(event) {
      ...
      if (msg.type === 'RELAY') {
        ...
        sender?.iframeRef.current?.contentWindow?.postMessage(
          { type: 'RECEIPT', ok: delivered, sent: delivered ? 1 : 0, failed: delivered ? [] : [msg.to] },
          '*'
        );
      }
    }
  ...
  return (
    <div className={styles.app}>
      <TopBar wsClient={wsClientRef.current} onBroadcast={handleTopBarBroadcast} isMasterActive={tasks.find(t => t.id === activeTaskId)?.role === 'master'} />
      <Sidebar
        tasks={tasks}
